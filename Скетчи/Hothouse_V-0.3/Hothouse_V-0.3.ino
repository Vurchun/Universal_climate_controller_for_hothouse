...
const char *pass =  "10072003WiFi"; // Пароль от точки доступа

const char *mqtt_server = " soldier.cloudmqtt.com"; // Имя сервера MQTT
const int mqtt_port = 12984 ;// Порт для подключения к серверу MQTT
const char *mqtt_user = "vvhzufvr" ;// Логи от сервер
const char *mqtt_pass = "yKN0yiP31mkt" ;// Пароль от сервера
const char *mqtt_id = "HotHouse" ;// Пароль от сервера

const char *host = "api.openweathermap.org";
const char *wapi =  "GET /data/2.5/weather?q=Kyiv,ua&appid=c557d50e325f16c912651e1054527de7 HTTP/1.1";
...
WiFiClient net; // Use WiFi_funkClient class to create TCP connections
...
void setup()
{
 ...
 Serial.print("checking wifi...");
  ...
  Serial.print("\nconnecting...");
  while (!client.connect(mqtt_id, mqtt_user, mqtt_pass)) {
    Serial.print(".");
    delay(1000);
  }
     Serial.println("Connected to MQTT server");
    client.subscribe("Hothouse/deltaHumiditi"); // подписывааемся по топик с данными для светодиода 
    client.subscribe("Hothouse/deltaT"); // подписывааемся по топик с данными для дельты температуры   
    client.subscribe("Hothouse/maxTempFanStart"); // подписывааемся по топик с данными для критичной температуры   
    client.subscribe("Hothouse/TimeFanWork"); // подписывааемся по топик с данными для работы вентилятора   
    client.subscribe("Hothouse/TimeIntervalFanWork "); // подписывааемся по топик с данными для периода вентилятора  
    client.subscribe("Hothouse/ControlHum"); // подписывааемся по топик с данными для заданной температуры
    client.subscribe("Hothouse/ControlTemp"); // подписывааемся по топик с данными для заданной влажности 
    client.subscribe("Hothouse/IfBankSave"); // подписывааемся по топик с данными для подтверждения перезагрузки  
    client.subscribe("Hothouse/NumBankSave"); // подписывааемся по топик с данными для банка загрузки 
     Serial.println("");
  ...
  lcd.setCursor(0, 2);  lcd.print("  By_Vurchun_V-0.3  ");
...
{jsonGet();
  ...
  delay(150);
    lcd.setCursor(18, 3); lcd.print("\8");           //Отправка значений на сервер    
                       ...
                       Serial.print(pub.topic()); Serial.print(" => "); Serial.print(pub.payload_string());// выводим в сериал порт название топика  // выводим в сериал порт значение полученных данных
                     String payload = pub.payload_string();
                     if(String(pub.topic()) == "Hothouse/NumBankSave") { NumBankSave = payload.toInt(); }// проверяем из нужного ли нам топика пришли данные // преобразуем полученные данные в тип integer
  
                         Serial.print(pub.topic()); Serial.print(" => "); Serial.print(pub.payload_string());// выводим в сериал порт название топика  // выводим в сериал порт значение полученных данных
                     String payload = pub.payload_string();
                     if(String(pub.topic()) == "Hothouse/IfBankSave") { IfBankSave = payload.toInt(); }// проверяем из нужного ли нам топика пришли данные // преобразуем полученные данные в тип integer
    ...
    IfBankSave = 0;
                      ...
                      Serial.print(pub.topic()); Serial.print(" => "); Serial.print(pub.payload_string());// выводим в сериал порт название топика  // выводим в сериал порт значение полученных данных
                     String payload = pub.payload_string();
                     if(String(pub.topic()) == "Hothouse/ControlTemp") { ControlTemp = payload.toInt(); }// проверяем из нужного ли нам топика пришли данные // преобразуем полученные данные в тип integer
  
                        Serial.print(pub.topic()); Serial.print(" => "); Serial.print(pub.payload_string());// выводим в сериал порт название топика  // выводим в сериал порт значение полученных данных
                     String payload = pub.payload_string();
                     if(String(pub.topic()) == "Hothouse/ControlHum") { ControlHum = payload.toInt(); }// проверяем из нужного ли нам топика пришли данные // преобразуем полученные данные в тип integer
   
                        Serial.print(pub.topic()); Serial.print(" => "); Serial.print(pub.payload_string());// выводим в сериал порт название топика  // выводим в сериал порт значение полученных данных
                     String payload = pub.payload_string();
                     if(String(pub.topic()) == "Hothouse/TimeIntervalFanWorkntrolTemp") { TimeIntervalFanWork = payload.toInt(); }// проверяем из нужного ли нам топика пришли данные // преобразуем полученные данные в тип integer

                        Serial.print(pub.topic()); Serial.print(" => "); Serial.print(pub.payload_string());// выводим в сериал порт название топика  // выводим в сериал порт значение полученных данных
                     String payload = pub.payload_string();
                     if(String(pub.topic()) == "Hothouse/TimeFanWork") { TimeFanWork = payload.toInt(); }// проверяем из нужного ли нам топика пришли данные // преобразуем полученные данные в тип integer 
    
                           Serial.print(pub.topic()); Serial.print(" => "); Serial.print(pub.payload_string());// выводим в сериал порт название топика  // выводим в сериал порт значение полученных данных
                     String payload = pub.payload_string();
                     if(String(pub.topic()) == "Hothouse/maxTempFanStart") { maxTempFanStart = payload.toInt(); }// проверяем из нужного ли нам топика пришли данные // преобразуем полученные данные в тип integer

                      Serial.print(pub.topic()); Serial.print(" => "); Serial.print(pub.payload_string());// выводим в сериал порт название топика  // выводим в сериал порт значение полученных данных
                     String payload = pub.payload_string();
                     if(String(pub.topic()) == "Hothouse/deltaT") { deltaT = payload.toInt(); }// проверяем из нужного ли нам топика пришли данные // преобразуем полученные данные в тип integer
  
                         Serial.print(pub.topic()); Serial.print(" => "); Serial.print(pub.payload_string());// выводим в сериал порт название топика  // выводим в сериал порт значение полученных данных
                     String payload = pub.payload_string();
                     if(String(pub.topic()) == "Hothouse/deltaHumiditi") { deltaHumiditi = payload.toInt(); }// проверяем из нужного ли нам топика пришли данные // преобразуем полученные данные в тип integer
  SaveToEEPROM(NumBankSave);
    }

    Serial.println("Start");
  client.publish("Hothouse/Temp",String(Temp));Serial.println(Temp);
  client.publish("Hothouse/ControlTemp",String(ControlTemp));Serial.println(ControlTemp);
  client.publish("Hothouse/Hum",String(Hum));Serial.println(Hum);
  client.publish("Hothouse/ControlHum",String(ControlHum));Serial.println(ControlHum);
  client.publish("Hothouse/Day",String(Day));Serial.println(Day);
  client.publish("Hothouse/Power",String(power));Serial.println(power);
  client.publish("Hothouse/NetPower",String(netpower));Serial.println(netpower);
...
case 1: {                                                                                                        // обработка события нажатия кнопки "МЕНЮ"
  ...
  case 0: {  lcd.clear();lcd.setCursor(0, 0);  lcd.print("T="); lcd.print(Tnow); lcd.print("\3 (");lcd.print(Temp);    lcd.print("\3)");  
             ...
             if( voltage >= 13.5 ){lcd.print("220V - ON"); }else{ lcd.print("Bat="); lcd.print(voltage);}
             lcd.setCursor(0, 2);
             ...
             BME280Read(); StartFan();;  StartHot();  StartHum(); TimerCalculatePrint();WiFi_F();FlagMenu = 0;             break; }
