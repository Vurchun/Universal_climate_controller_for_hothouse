...
void setup()
{
 ...
 Serial.print("checking wifi...");
  ...
  digitalWrite(PinHot, HIGH);  digitalWrite(PinLite, HIGH);  digitalWrite(PinHum, HIGH); digitalWrite(PinHumGroundControl, HIGH);  // первоначальное отключение всех устройств при загрузке с паузой в 0,2сек чтобы не было скачка падения напряжения на БП
  ...
  lcd.setCursor(0, 2);  lcd.print("  By_Vurchun_V-0.4  ");
...
void SoilMoistureRead(){
  ...
  HumGroundnow = map(HumGroundnow, MIN, MAX, 0, 100);  // адаптируем значения от 0 до 100,
...
{jsonGet();
  ...
  client.publish("Hothouse/Temp",String(Tnow));Serial.println(Tnow);
  client.publish("Hothouse/ControlTemp",String(Temp));Serial.println(Temp);
  client.publish("Hothouse/Hum",String(hum));Serial.println(hum);
  client.publish("Hothouse/ControlHum",String(Humiditi));Serial.println(Humiditi);
  client.publish("Hothouse/Power",String(voltage));Serial.println(voltage);
...
case 1: {                                                                                                        // обработка события нажатия кнопки "МЕНЮ"
  ...
  case 0: {  lcd.clear();lcd.setCursor(0, 0);  lcd.print("T="); lcd.print(Tnow); lcd.print("\3 (");lcd.print(Temp);    lcd.print("\3)");  
             ...
             BME280Read(); StartFan();SoilMoistureRead();StartHot();StartHum(); TimerCalculatePrint();WiFi_funk();FlagMenu = 0;             break; }
