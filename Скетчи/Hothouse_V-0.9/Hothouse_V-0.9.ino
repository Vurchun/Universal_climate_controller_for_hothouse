...
case 1: {                                                                                                        // обработка события нажатия кнопки "МЕНЮ"
  ...
  case 0: {  lcd.clear();lcd.setCursor(0, 0);  lcd.print("T="); lcd.print(Tnow); lcd.print("\3 (");lcd.print(Temp);    lcd.print("\3)");  
             ...
             StartFan( Temp, deltaT, maxTempFanStart, FanWorkFlag, FlagTimeFan, TimeFaning, TimeIntervalFaning, currentMillis);
             StartHot(Temp, deltaT, PinHot, Output, consKp, consKi, consKd);                           
             StartHum(TypeHouse, hum, Humiditi, deltaHuminiti, HumGtound, PinHumGroundControl);
             StartLite(PinLite);           TimerCalculatePrint();WiFi_funk();                                                                                                                                                                   FlagMenu = 0;   break; }
